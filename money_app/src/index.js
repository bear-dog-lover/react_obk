...
// Uncontrolled Components version
class MoneyEntry extends Component {
  constructor(props) {
    super(props)
    this.date = null
      ...
      <div className="entry">
        <fieldset>
          ...
          <div>
            ...
            <input type="radio" name="payingIntOut" />出金
          </div>
          <div>日付: <input type="text" ref={(node) => this.date = node} /></div>
          <div>項目: <input type="text" ref={(node) => this.item = node} /></div>
          <div>金額: <input type="text" ref={(node) => this.amount = node} /></div>
          <div><input type="submit" value="追加" onClick={() => this.onClickSubmit()} /></div>
        </fieldset>
      </div>
    )
